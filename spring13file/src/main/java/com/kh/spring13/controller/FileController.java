...
public class FileController {
   ...
   //	파일 다운로드를 위해서 알아야 할 내용  2023.02.17
 ...
 // 
   	...
   	public ResponseEntity<String> download2(){
   		//ResponseEntity의 능력들
   		//[1] 요청 결과 형태를 정의 할 수 있다 
   		
//      이렇게 하면 요청 성공(ok=200) + download2를 반환하는 명령   		
//   	return ResponseEntity.ok("download2");
//		return ResponseEntity.ok().build(); 이러면 근데 바디 못만듬    		
...
//   	- Apache commons IO 라이브러리를 사용할거임.
//		- 헤더에 다음 정보를 설정한다.
...
//			- 이후 바디에 파일의 실제 데이터를 첨부한 뒤 반환하면 스프링이 나머지는 처리해준다.   	
