...
//	private PocketmonDao pocketmonDao;
	...
	@GetMapping("/")
		public List<PocketmonDto> list(){
			return pocketmonRepository.selectList();
		}
	
	@GetMapping("/{no}")
		public String find(@PathVariable int no) {
			PocketmonDto dto = pocketmonRepository.selectOne(no);
			if(dto == null) {//없다 = 사용가능
				return	 "NNNNY";
			}
	...
	public void insert(@ModelAttribute PocketmonDto pocketmonDto) {
		pocketmonRepository.insert(pocketmonDto);
	}
	
	@DeleteMapping("/{no}")
	public void delete(@PathVariable int no) {
		pocketmonRepository.delete(no);
	}
...
//	@PatchMapping("/") // 일부 정보를 바꿀 때 이름, 타입 바꿀때는 그럼 패치 2개 만들어야함
	public void edit(@ModelAttribute PocketmonDto pocketmonDto) {
		pocketmonRepository.update(pocketmonDto);
